...
| **Stochastic gaze policy** | `foveated_stochastic_gibson` (FoveatedStochasticGazePolicy) | Queued for collaborator's hc cluster (see `experiments/hc_experiment_plan.md`) | Reparameterized bounded-σ Gaussian gaze sampling; designed to fix gaze collapse without aux loss. Re-tests H3 with a working learned gaze. |
