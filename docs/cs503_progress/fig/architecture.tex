% Architecture diagram — include with \input{fig/architecture.tex}
...
% Matched-compute note
\node[font=\tiny\itshape, text=blue!55, below left=0.05cm and -0.1cm of rgb]
    {Matched: lower res};

\end{tikzpicture}%
}
\caption{Shared agent architecture across all five experimental conditions.
Solid blue modules form the visual pipeline (absent in the \emph{Blind} condition).
The dashed-orange foveation module applies Gaussian blur with quadratic fall-off
and is present only in the foveated conditions.
The dotted-red gaze decoder feeds the predicted fixation centre back to the
foveation module and is exclusive to \emph{Fov-learned}.
All other modules (grey) are identical across conditions.
Training uses DD-PPO.}
