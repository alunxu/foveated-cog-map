...
Uniform     & 400M & 400M & 97.6\% & 0.805 & \textbf{Complete} \\
...
\begin{table}[h]
\centering
\caption{Shortcut / cognitive-map behavioral test (20 scenes $\times$ 10 episodes per condition). $\Delta$ = persistent $-$ reset.}
\label{tab:shortcut}
\begin{tabular}{lccccc}
\toprule
 & \multicolumn{2}{c}{Reset (standard)} & \multicolumn{2}{c}{Persistent} & $\Delta$ Success \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
Condition & Success & SPL & Success & SPL & \\
\midrule
Blind    & 0.78 & 0.432 & 0.24 & 0.145 & \textbf{$-$0.53} \\
Matched  & 0.86 & 0.511 & 0.75 & 0.421 & $-$0.12 \\
Uniform  & \textbf{0.95} & \textbf{0.786} & \textbf{0.93} & \textbf{0.592} & $-$0.02 \\
Foveated & \multicolumn{5}{c}{\textit{Pending (resubmitted after grid-buffer fix)}} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[nosep,leftmargin=1.5em]
    \item \textbf{Persistent memory hurts all conditions}, but the severity varies by an order of magnitude: blind $-$53\%, matched $-$12\%, uniform $-$2\%.
    \item \textbf{Vision acts as a grounding signal.} Sighted agents---especially uniform---maintain near-perfect success even with out-of-distribution hidden states. The visual stream provides real-time spatial evidence that overrides stale memory, preventing the catastrophic failure seen in blind.
    \item \textbf{SPL drops even when success holds.} Uniform loses $-$0.194 SPL despite only $-$2\% success loss, indicating that accumulated state increases path inefficiency (more wandering) without preventing goal-reaching.
    \item \textbf{Map-building is negative for all conditions.} Within the persistent condition, later episodes are \emph{worse} than earlier ones (blind: $-$0.146, matched: $-$0.127, uniform: $-$0.112), confirming progressive state corruption rather than spatial learning.
\end{enumerate}

\textbf{Interpretation:} No condition builds a reusable cognitive map across episodes. The blind agent's LSTM representations are \emph{episode-scoped path-integration states}: ``where I am relative to where I started this episode.'' Carrying this across episode boundaries places the LSTM in an out-of-distribution regime, collapsing performance. Sighted agents degrade more gracefully because visual input re-anchors the hidden state each step, but even they do not \emph{benefit} from persistent memory.
...
Cognitive map (behavioral) & \textbf{Negative (all)} & Persistent LSTM hurts all agents; \\
 & & blind $-$53\%, uniform $-$2\%. \\
 & & Vision grounds but doesn't build maps \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[nosep]
    \item \textbf{Complete training} (in progress): Foveated (155M$\to$250M), fov-learned (45M$\to$250M). Blind, uniform, and matched complete.
    \item \textbf{Foveated shortcut eval} (queued): Resubmitted after grid-buffer fix. Will complete the cross-condition behavioral comparison.
