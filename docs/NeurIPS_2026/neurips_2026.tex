...
\noindent\textbf{Layer 0 sensor pass-through is shared; the divergence is at Layer 2}\quad All five conditions decode GPS at Layer 0 (Figure~\ref{fig:h1_mega}c) because the GPS sensor embedding enters the LSTM input vector there directly. The H1 distinction is what happens through Layers 1--2: bottleneck conditions retain the code to the top layer, rich-encoder conditions discard it. The standard DD-PPO action head is a linear projection from $\mathbf{h}_2$, so ``linearly decodable at Layer 2'' maps to ``policy-accessible''. \emph{(Side observation: blind's Layer 1 R² is lower than Layer 0 or Layer 2; not yet explained.)}

\noindent\textbf{Non-linear probe at the top layer recovers position}\quad A 2-layer MLP on rich-encoder top-layer states recovers GPS at $R^2 \in [0.51, 0.73]$. The bottleneck-vs-pass-through divergence is therefore not about whether any probe can recover position --- it is specifically about whether a \emph{linear, top-layer} probe can. We cannot from MLP recovery alone exclude exploitation of features merely correlated with position (wall distance, scene identifiers, episode-phase); §\ref{sec:foveation} and Appendix~\ref{app:scaling} test the bottleneck framing more directly.

\noindent\textbf{MP3D dataset shift}\quad The bottleneck-vs-pass-through partition is preserved on held-out MP3D (Figure~\ref{fig:h1_mega}a, MP3D bars). Two single-seed cross-dataset shifts are worth flagging: foveated GPS recovers from chance to $+0.35$ on MP3D, and foveated-learned compass shows a $+1.75$ Gibson$\to$MP3D swing. Both are candidates for Gibson-specific feature use by the foveated encoder; multi-seed replication needed before attributing to a specific mechanism. Compass numbers in Appendix~\ref{app:supp_figs}.
...
The four in-flight experiments below test \emph{which} aspect of foveation drives these differences --- sensor transform strength, spatial-sampling vs.\ blur, gaze location, or encoder feature-map representation. Numbers will populate the sub-paragraphs in revision; current status in Appendix~\ref{app:foveation-status}.
...
\noindent\textbf{What we already conclude}\quad From existing data (Table~\ref{tab:fov-vs-uni}), foveation under $\sigma_{\max}=8$ Gaussian blur is \emph{not a copy of uniform}: it converges with uniform on H1 but diverges on H2 (subspace orthogonality, 1-NN separability), behavioural memory reliance (shortcut drop), transplant compatibility, and dataset-shift recovery. The encoder feature-map probe further shows the encoder discards linear position regardless of condition --- so \emph{whatever foveation does differently from uniform, it does it inside the LSTM in subspaces invisible to a linear-GPS probe}. The in-flight experiments below probe \emph{what} that ``something different'' is.
...
Figure~\ref{fig:shortcut_paired_traj} shows the failure modes qualitatively differ across regimes: blind's persistent trajectory clusters around the previous-episode goal (literal ``locked onto old position''); uniform and foveated wander diffusely (``locked onto old visual context'' would fit). The visualisation cannot itself distinguish position-locked from context-locked failure but is consistent with the 2$\times$2 dissociation. \TODO{Both anomalies rest on single-seed runs; multi-seed replication is in progress and would promote the dissociation from candidate to robust. A direct ``policy GPS reliance'' test (ablating the GPS sensor mid-rollout) would distinguish ``policy reads the GPS code'' from ``policy reads non-GPS memory'' --- the matched anomaly predicts a small SPL drop, the uniform anomaly a large one.}
...
\emph{Task competence.} All conditions solve PointGoal at $93$--$99\%$ success; the encoded-content gap is not a skill difference.

\emph{Probe capacity / fitting.} DtG decodes robustly everywhere ($R^2 \geq 0.81$), demonstrating Ridge can fit high-$R^2$ scalars when the hidden state contains them. Hewitt--Liang label-permutation selectivity is high where GPS is high (blind, matched) and at chance where GPS is at chance --- the latter follows from real and control probes facing the same near-zero target-information ceiling. Rich-encoder chance-level R² is therefore a hidden-state property, not a probe artefact.

\emph{Sample size / trajectory distribution.} Identical 500-episode deterministic evaluation across conditions; the earlier 4-step stochastic-sampling artefact is documented and retired in §\ref{sec:methods-probing}.

\emph{Linear-probe artefact.} Two complementary checks. \emph{(a)~Behavioural:} memory transplant goes outside the probe framework and reproduces the bottleneck-vs-pass-through ordering; the $5\times5$ matrix's asymmetry adds a structure that no linear-similarity measure can read. \emph{(b)~Non-linear probe:} a 2-layer MLP recovers $R^2 \in [0.51, 0.73]$ from rich-encoder top-layer states where Ridge sees chance. The bottleneck-vs-pass-through divergence is therefore not about whether \emph{any} probe can recover position; it is specifically about whether a \emph{linear} top-layer probe can. Since the DD-PPO action head is a linear projection from $\mathbf{h}_2$, the linear metric is what the policy itself can use, so H1 maps onto policy-accessible encoding regardless of how the MLP interpretation resolves. We cannot from MLP recovery alone exclude exploitation of position-correlated features (wall distance, scene identifiers, episode-phase) rather than position itself; Appendix~\ref{app:scaling} tests this more directly.
