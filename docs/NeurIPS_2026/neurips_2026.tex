...
\noindent\textbf{Proposed mechanism}\quad We \emph{propose} the following account, consistent with the data but not directly tested. In PointNav, GPS and compass are observations the policy reads each step; the recurrent state need only re-encode them if it cannot rely on the visual stream to do so. A full-resolution RGB encoder can re-derive position and heading from the current frame --- so as the episode progresses and the rich-encoder LSTM accumulates other task-relevant features (visual landmarks, scene history, goal-direction integration), it has no incentive to keep refreshing the GPS code, and the top-layer state drifts off it. An impoverished encoder cannot re-derive position from vision alone; the LSTM has to keep the GPS code linearly readable for the policy throughout the episode, and the top-layer code stays stable. Blind and matched-compute bracket this picture: blind has \emph{no} visual encoder (pure compensation); matched has a \emph{collapsed} $1{\times}1$ encoder (partial compensation); foveated, uniform, and foveated-learned all have full encoders (pass-through). Task success is tightly bunched, so a skill-gap artefact is unlikely to explain the encoding gap. The temporal dynamic in Figure~\ref{fig:h1_mega}b (rich-encoder GPS code emerging early then decaying) is the direct fingerprint of this account. \TODO{A direct causal test of the mechanism --- e.g., training a rich-encoder agent with GPS perturbed mid-rollout, or training a bottleneck agent with the GPS sensor removed --- is left for future work.}

\noindent\textbf{Relation to Wijmans et al.}\quad Wijmans et al.~\citep{wijmans2023emergence} showed that blind PointNav agents develop spatial representations in LSTM memory. Our blind result ($R^2=0.95$) replicates this; matched-compute extends it beyond the blind special case to a condition that has visual input but cannot resolve world-frame structure from it. We interpret this as evidence that the relevant trigger is the visual encoder's information capacity, not the binary presence/absence of vision. §\ref{sec:foveation} and Appendix~\ref{app:scaling} test the trigger directly.
...
\includegraphics[width=\linewidth]{fig/transplant_sweep.pdf}
\end{minipage}
\caption{H2 convergent evidence. \emph{Left:} Unaligned linear CKA~\citep{kornblith2019cka} between top-layer LSTM hidden states ($n{=}95{,}640$ steps per condition under deterministic-rollout probes); every off-diagonal is below $10^{-4}$. \emph{Right:} Memory transplant across midpoints $\{0,15,30,60,90\}$ ($n=100$--$150$ episodes/cell). \emph{(a)}~Baseline / self-transplant / cross-transplant SPL; at $t{=}0$ self- and cross- reduce to a protocol-noise floor. \emph{(b)}~Cross minus self (the isolated condition-mismatch component) is roughly stable for $t \geq 15$: foveated-learned$\to$foveated incurs $-0.17$ SPL on average even though all pairwise CKA values sit at the noise floor. Probe-based similarity is uninformative at this scale; transplant reveals the functional gap.}
\label{fig:h2}
\end{figure}
...
\noindent\textbf{Lead with the behavioural test}\quad The most informative H2 measurement is also the most direct test of ``do these representations matter?'': \emph{memory transplant} (Figure~\ref{fig:h2}, right). For each donor--recipient pair, the donor drives the first $t$ steps of an episode, its top-layer hidden state is copied into the recipient at step $t$, and the recipient's policy drives the rest. The isolated condition-mismatch cost --- cross-transplant SPL minus self-transplant SPL, which subtracts both a small $t{=}0$ protocol-noise floor and the rollout-divergence cost of mid-episode state replacement --- is roughly stable across midpoints $t \geq 15$ and carries a clear ranking: foveated-learned$\to$foveated incurs $-0.17$ SPL, foveated$\to$uniform $-0.10$, and foveated$\to$blind essentially neutral ($-0.02$). Pairs that look identically near-zero in linear-similarity measures differ markedly under transplant. Probe-based similarity is uninformative at this scale; the functional gap is what matters. \TODO{The remaining cross-condition transplant pairs (e.g., blind$\to$matched, matched$\to$uniform) are not yet measured; running them would let us state the format-divergence finding as a complete $5{\times}5$ behavioural matrix rather than a representative subset.}
