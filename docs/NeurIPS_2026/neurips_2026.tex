...
\subsection{Foveation: where it sits, and where the cleaner tests sit}
\label{sec:foveation}

The five-condition results above place our two foveated variants (foveated-fixed, foveated-learned) in the rich-encoder pass-through regime alongside uniform: under $\sigma_{\max} = 8$ Gaussian-blur foveation with the same ResNet-18 encoder stack, the LSTM does not develop a stable top-layer GPS code (Figure~\ref{fig:h1_mega}). This is itself a finding --- it bounds where a soft Gaussian-blur foveation falls on the encoder--memory race spectrum --- but it leaves three sharper questions about foveation that the present five conditions cannot resolve. We submitted controlled experiments for each; the following four sub-paragraphs describe the experimental design and what the result will mean. Numbers will populate this subsection in revision; current status is tracked in Appendix~\ref{app:foveation-status}.
...
\noindent\textbf{Spatial sampling vs.\ blur (F3 log-polar control, in progress)}\quad Gaussian blur removes high-frequency content but preserves spatial layout: a foveated 256$\times$256 image still has 256$\times$256 pixels feeding the encoder, which can still resolve coarse position from peripheral structure. A log-polar foveation~\citep{strasburger2011peripheral,rosenholtz2016peripheral} resamples the image with a non-uniform spatial grid: high spatial sampling at the fovea, low sampling at the periphery. This is the architectural commitment that ``foveated vision'' makes in biology. We train an agent with the same ResNet-18 stack but a log-polar input, and ask whether \emph{spatial-sampling foveation} (rather than blur-only foveation) triggers the encoder--memory race. If yes, it sharpens H1 from a ``bottleneck of any kind'' claim to a ``spatial-resolution bottleneck specifically'' claim.
...
\noindent\textbf{What we can already conclude about foveation}\quad From the existing data alone (i.e.\ before F1--F4 / log-polar / fov-shifted land), four foveation findings are already supported. \emph{(a)~Behavioural placement.} Foveated-fixed solves PointGoal at $0.78$ SPL / $96\%$ success (Table~\ref{tab:summary}) and traces a near-direct path through the corridor on a typical episode (Figure~\ref{fig:setup}d, $80$ steps vs.\ blind's $231$ and matched's $146$), behaviourally indistinguishable from uniform on the same episode. \emph{(b)~Probe placement.} Top-layer LSTM GPS $R^2 = +0.06 \pm 0.88$, compass $+0.07 \pm 0.69$ --- both at chance, in the rich-encoder pass-through regime alongside uniform (Figure~\ref{fig:h1_mega}a). \emph{(c)~Encoder placement.} The encoder feature map cannot linearly decode world-frame GPS ($R^2 = -0.65$, Figure~\ref{fig:encoder_probe}) --- soft Gaussian-blur foveation does not produce a useful encoder bottleneck. \emph{(d)~Functional non-equivalence to uniform despite indistinguishable probe placement.} A foveated$\to$uniform memory transplant incurs $-0.10$ SPL beyond the self-transplant noise floor (Figure~\ref{fig:h2}), and the two conditions occupy non-overlapping linear subspaces (CKA noise floor, 1-NN purity $1.000$). \emph{Together, these establish a baseline that the in-flight experiments build on}: any genuine foveation-specific effect must show up either in (i) the strength sweep (low- vs high-$\sigma_{\max}$ behaviour), (ii) the log-polar control (spatial-sampling vs.\ blur), or (iii) the foveated-shifted control (gaze location); the present $\sigma_{\max}=8$ Gaussian-blur version does not.
