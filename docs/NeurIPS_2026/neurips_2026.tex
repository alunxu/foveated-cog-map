...
\emph{Boundaries.} Our similarity tests are linear or near-linear; non-linear measures could reveal higher-order shared structure. Our five sensors are diverse but not exhaustive; hybrid sensors (sharp fovea + low-acuity periphery, depth-only) might produce intermediate geometries.


\subsection{Foveation: convergence on H1, divergence on H2 + behaviour}
\label{sec:foveation}

Foveated (fix) and uniform fall in the same rich-encoder pass-through regime on the headline H1 metric (top-layer GPS $R^2$ at chance), but they are not interchangeable on H2 / behavioural / dataset-shift axes (Table~\ref{tab:fov-vs-uni}). The take-away: \emph{within the rich-encoder regime defined by H1, foveation and uniform produce internally distinct representations whose differences only the H2-and-beyond machinery makes visible}.
...
Two takeaways follow. \emph{The H1 phenomenon lives inside the LSTM, not at the encoder.} Probing the encoder feature map directly shows that no sighted encoder linearly decodes GPS (Figure~\ref{fig:h1_mega}d, grey bars), so the bottleneck-vs-rich-encoder gap is not encoder-level position retention. \uncertain{The LSTM-vs-encoder gap (coarse gains $\sim 3.9$, foveated $\sim 0.7$, uniform $\sim 0$) orders conditions by how much the LSTM contributes \emph{above} what the encoder already supplies; we interpret this as a gradient in how much the LSTM relies on visual features rather than integrated GPS-sensor signal as encoder spatial-feature variety grows.} \emph{Foveation is not a copy of uniform.} Even where they converge on the H1 metric, the encoder feature-map probe rules out encoder-level differences --- so \uncertain{whatever foveation does differently from uniform happens inside the LSTM in subspaces a linear-GPS probe is blind to.} The four in-flight foveation experiments (Appendix~\ref{app:foveation-status}) test \emph{what} that something is; F3 log-polar in particular makes a falsifiable prediction: a $\sim 2{\times}2$ encoder spatial output should produce LSTM GPS $R^2 \geq 0.3$, between coarse's $+0.78$ and uniform's $\approx 0$. If instead it matches uniform, the encoder-spatial-output mechanism would need reframing.\pendnote{This interpretation is consistent with the data but not directly tested by a causal intervention; the resolution scaling sweep (Appendix~\ref{app:scaling}) tests it. All ``LSTM gain'' numbers and the foveation-vs-uniform contrasts in Table~\ref{tab:fov-vs-uni} are single-seed.}
...
Three probes that do \emph{not} discriminate define the boundary of our claims. Low-resolution (visited-region) occupancy decodes above chance everywhere; metric $20{\times}20$ occupancy fails everywhere (consistent with~\citep{ramakrishnan2025space} that high-resolution metric layout needs non-linear probes); the ego-frame goal vector is at chance everywhere because PointGoal supplies it as a per-step input. Population coding (Appendix~\ref{app:supp_figs}) sharpens H1 in a direction we did not expect: rich-encoder conditions have a few units with \emph{higher} peak spatial information than blind, but those peaked units do not decode GPS. \uncertain{This suggests rich-encoder peaked units encode features correlated with position (visual landmarks, trajectory phase) rather than position itself; blind's broader, lower-peak distribution carries the linearly-decodable position code.}
