...
\begin{abstract}
Does the structure of an agent's visual sensor reshape what its recurrent memory encodes? Holding task, optimiser, and LSTM backbone identical and varying only the visual pathway across five conditions (blind, uniform, foveated with fixed and learned gaze, and a matched-compute control whose encoder spatial output collapses to $1{\times}1$), we probe the recurrent state for world-frame position. We find an \emph{encoder--memory race}: when the encoder hands the LSTM minimal spatial-feature variety per step, the LSTM maintains a linearly-decodable top-layer GPS code; when the encoder supplies richer spatial features, that code is overwritten as the episode progresses. This recasts ``cognitive maps emerge without vision''~\citep{wijmans2023emergence} as ``cognitive maps emerge --- and persist --- when the visual pathway cannot resolve the map on its own.'' Independently, the five agents solve the task at $93$--$99\%$ success but develop hidden states in linearly disjoint subspaces (CKA $<10^{-4}$, $1$-NN purity $1.000$); cross-condition memory transplants incur task cost beyond transplant mechanics. The pattern reproduces on held-out MP3D and parallels three threads of animal-navigation research --- enhanced hippocampal coupling in blind humans, grid-cell collapse in darkness, cross-modality hippocampal remapping in bats. An encoder-resolution scaling sweep that would test the H1 mechanism causally is in progress.
\end{abstract}
...
The empirical findings, in order of evidence strength. \emph{(i) Encoder--memory race.} All five conditions encode top-layer GPS in the typical-episode window, but only the bottleneck conditions (blind; matched-compute, $1{\times}1$ encoder feature map) \emph{maintain} the code into the long-tail; rich-encoder conditions overwrite it. Direct probing of the encoder output sharpens the mechanism: no sighted encoder linearly decodes GPS, so H1 lives inside the LSTM and the trigger is encoder spatial-feature \emph{variety} per step, not encoder position retention. The ordering reproduces on held-out MP3D. \emph{(ii) Format divergence.} The five agents solve the task at the same success rate but their hidden states occupy linearly disjoint subspaces (CKA $<10^{-4}$, $1$-NN purity $1.000$); cross-condition memory transplants are asymmetric, with bottleneck donors hurting rich-encoder recipients far more than the reverse. \emph{(iii) Foveation $\neq$ uniform.} Under $\sigma_{\max}=8$ Gaussian-blur foveation, foveated and uniform agents agree on H1 readout but diverge on H2, behaviour, and dataset shift; whatever foveation does differently lives in subspaces invisible to a linear-GPS probe. \emph{(iv) Probe-readable vs.\ policy-used memory dissociates.} Plotting H1 against behavioural memory reliance gives a $2{\times}2$ with two opposite anomalies: matched has linear GPS but barely uses it; uniform has no linear GPS but heavily relies on memory. \emph{(v) H3 (gaze location)} is tested by a foveated-shifted control in training at submission.
