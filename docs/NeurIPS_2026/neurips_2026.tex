...
A trajectory-level look at \emph{how} persistent-memory failures manifest is consistent with the dissociation (Figure~\ref{fig:shortcut_paired_traj}). Aggregating across all same-floor paired-episode failures (Figure~\ref{fig:shortcut_paired_traj}, right panel): uniform is the only condition with a meaningful sample size whose persistent agent ends up closer to the previous-episode goal than the new one (margin $+1.83$m on $n{=}46$); blind / matched / foveated all end nearer the new goal but never reach it. Foveated-learned shares uniform's positive-margin direction at $+2.30$m but $n{=}5$ is too small to interpret. \emph{If} the uniform pattern survives multi-seed replication, it is consistent with the $2{\times}2$: uniform's memory would then plausibly act as a visual / scene-history anchor that locks the agent in place, whereas blind's GPS code would interfere via a different channel (the recurrent state mis-reporting current position to the policy's goal-direction integrator). The locking interpretation is single-seed and we do not claim it has been established; multi-seed replication is in flight.
