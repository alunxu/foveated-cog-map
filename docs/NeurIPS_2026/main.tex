...
\begin{itemize}
  ...
  \item \textbf{Cross-condition quantitative comparisons} (§\ref{sec:baseline} baseline probes, §\ref{sec:h2} CKA and probe transfer, §\ref{sec:h3} goal-vector and heading-content claims, §\ref{sec:addl} MP3D) use \emph{truncated-to-first-4-steps} for foveated-learned and full episodes for the other four; the truncation matches the natural horizon of the other conditions. Tables and figures are labelled ``truncated'' where this applies.
  \item \textbf{Within-condition analyses where foveated-learned is not compared against others} (§\ref{sec:addl} per-condition place-cell counts, §\ref{sec:addl} multi-layer heatmap) use each condition's full-episode data.
  \item \textbf{Behavioural interventions} (§\ref{sec:h2} transplant, §\ref{sec:h3} shortcut) use full rollouts for every condition because they measure on-policy task performance, which cannot be meaningfully truncated.
  \item \textbf{Path-history lag-$k$ probe} (§\ref{sec:h1}) requires $k+1$ consecutive steps per episode; with $k=5$ the 4-step truncation would exclude every sample. Foveated-learned is therefore \emph{excluded from H1} rather than truncated, and we discuss the absent data point separately.
\end{itemize}
...
\paragraph{Why fov-learned is not in this comparison.}
...
\paragraph{Strong form: not supported, though not decisively refuted.}
...
\paragraph{Content-steering form: supported by a categorical representational shift.}
...
\paragraph{Mechanism: the dissociation is about what accumulates, not about what is missing.}
...
\paragraph{Controls: what does not separate conditions.}
