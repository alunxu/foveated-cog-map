...
We evaluate three donor$\rightarrow$recipient pairs across a midpoint sweep $\{0, 15, 30, 60, 90\}$ steps ($n=100$--$150$ episodes/cell). For each pair we compare three conditions: (a)~\emph{baseline}, the recipient driving the full episode alone; (b)~\emph{self-transplant}, the recipient driving the first $t$ steps, its own recurrent state being copied and re-inserted at step $t$, then driving the rest; (c)~\emph{cross-transplant}, the donor driving the first $t$ steps and its recurrent state being pasted into the recipient at step $t$. Three observations from the sweep (Figure~\ref{fig:transplant}) pin down the behavioural signature of H2. First, the $t=0$ protocol-noise floor is small: at midpoint $0$ the donor accumulates no information yet and we observe self-transplant $-0.03$ SPL and cross-transplant $-0.06$ SPL vs.\ baseline, small residual drifts attributable to stochastic-action sampling. Second, self-transplant SPL drops monotonically with midpoint (foveated$\rightarrow$uniform: $0.79$ at $t=15$ to $0.29$ at $t=90$) and the additional drop beyond the $t=0$ floor is real rollout divergence --- the recipient's own past recurrent state genuinely diverges from the state it would have occupied had it driven the rollout from step 0. This means the raw ``cross $-$ baseline'' gap conflates condition mismatch with rollout divergence; subtracting self-transplant SPL isolates the pure condition-mismatch effect. Third, that isolated effect (Figure~\ref{fig:transplant}b) is roughly stable across midpoints $t\geq 15$: foveated-learned$\rightarrow$foveated sits around $-0.17$ SPL (range $-0.09$ to $-0.20$), foveated$\rightarrow$uniform around $-0.10$ SPL (range $-0.09$ to $-0.12$), foveated$\rightarrow$blind around $-0.02$ SPL. The striking entry is foveated-learned$\rightarrow$foveated --- the pair with the highest pairwise CKA of any condition pair ($0.0035$, Figure~\ref{fig:cka}) --- being the \emph{most} behaviourally incompatible pair, showing that higher probe-based similarity does not predict higher functional interchangeability when sensor structure or gaze policy differs. Blind's near-zero drop at all midpoints is the complementary observation: blind's policy barely uses recurrent content, so what donor state is pasted in hardly matters. Cross-condition recurrent memories are not a generic substrate any recipient can consume.
