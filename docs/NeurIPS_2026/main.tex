...
\begin{abstract}
What shapes the format of a recurrent spatial representation---the task, the architecture, or the structure of the visual input? We train DD-PPO PointNav agents in Habitat under five visual conditions that differ only in sensor structure and gaze policy (blind, uniform, foveated with fixed gaze, matched-compute, foveated with learned gaze). All sighted conditions solve the task; probing their recurrent memory with linear decoders, representational-similarity analysis, probe transfer, and two behavioural interventions on the recurrent state (memory transplant and shortcut discovery) reveals that sensor structure and gaze policy are two orthogonal, causally-linked axes that shape the \emph{format}---not merely the strength---of the learned spatial representation. Non-uniform vision induces compensatory temporal memory that a matched-pixel uniform control does not recreate. Different sensor-structure and gaze-policy combinations produce representationally disjoint recurrent geometries that are not interchangeable via transplant, even between two agents that share the same foveation transform. A collapsed learned gaze steers the representation from position-dominant to heading-dominant content without altering task performance. Each of these effects has a direct analog in the primate, rodent, and bat spatial-memory literatures, which we take as convergent evidence that the findings reflect general sensor--memory coupling principles rather than artefacts of our specific network. For visual-intelligence practitioners, the core implication is that sensor design and learned attention policies are not task-performance-only choices: they are representational-content axes whose downstream effects on what agents' recurrent memories can encode are measurable and, in principle, tunable.
\end{abstract}
