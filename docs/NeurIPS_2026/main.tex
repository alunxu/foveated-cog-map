...
\begin{abstract}
Map-like representations emerge in the recurrent memories of navigation agents trained with deep RL, but prior work has trained these agents only under blind or spatially uniform perception~\citep{wijmans2023emergence,dwivedi2022isee}. In biological vision, input is never uniform; primates code joint place--view--action state precisely because they sample the scene with a foveated sensor~\citep{wirth2017gaze,rolls2024why}. This gap motivates a controlled ML question: when visual input \emph{structure} is the only free variable in otherwise identical navigation agents, does the \emph{format} of learned spatial memory change with it, or only its strength? We train DD-PPO PointNav agents in Habitat under five visual conditions---blind, uniform, foveated (fixed centre), matched-compute, and foveated (learned gaze)---and analyse the resulting recurrent memory with linear probes, cross-condition CKA, probe transfer, per-unit spatial-information measures, and two behavioural interventions directly on the recurrent state (memory transplant and shortcut discovery). Three findings emerge. \emph{(1)~Compensatory memory.} Spatially non-uniform vision induces longer-lived spatial traces: the foveated agent retains decodable position information five steps into the past ($R^2=0.56$) while the uniform baseline's probe collapses ($R^2=-1.5$); the matched-compute control rules out total information volume as the explanation. \emph{(2)~Sensor-dependent representational format.} Each visual condition produces a distinct representational geometry (CKA $<0.004$ between all pairs; catastrophic probe transfer) despite $>97\%$ task success; pasting a donor's mid-episode hidden state into a different-condition recipient drops SPL by up to $0.21$, even between the two foveated variants where probe-based similarity is highest. \emph{(3)~Gaze-locked representational content.} The learned-gaze agent collapses to a near-constant gaze and, as a result, builds a content-shifted representation: highest compass decoding of any condition ($R^2=0.94$) but near-chance GPS decoding ($R^2=0.03$); carrying memory across episodes with a new goal hurts it least ($15\%$ relative SPL drop vs.\ $41$--$52\%$ for uniform/blind). Each finding has a direct biological precedent~\citep{kupers2014blind,chen2016grid,geva2016remapping,jutras2013saccades}, which we discuss as evidence that the effects reflect general principles of sensor-memory coupling rather than architectural artefacts of our specific network. Together the three findings establish sensor structure and gaze policy as controllable, causal axes along which learned spatial representations can be steered---axes that are invisible under the uniform-vision assumption made by prior work.
\end{abstract}


\section{Introduction}

Wijmans et al.~\citep{wijmans2023emergence} showed that DD-PPO agents trained on PointGoal navigation develop map-like representations in their recurrent hidden states---even \emph{blind} agents receiving only goal displacement, GPS, compass, and a close-to-goal scalar; Dwivedi et al.~\citep{dwivedi2022isee} probed sighted agents for related spatial concepts. These studies have been decisive in showing that spatial codes emerge from task pressure alone, but they share one methodological limitation: they all assume \emph{spatially uniform visual certainty}. In biological vision this assumption is never met. Primates see sharply only within $\sim$2\textdegree{} of gaze, and primate hippocampal cells accordingly code joint place--view--action state rather than location alone~\citep{wirth2017gaze,rolls2024why}; the same bat, in the same environment, produces non-overlapping hippocampal populations when it alternates between vision and echolocation~\citep{geva2016remapping}. If visual-input structure shapes what biological memory represents, it very likely shapes what artificial recurrent memory represents too---but this has not been tested, because the controlled ablation it requires (same architecture, same task, same training budget, only sensor structure varied) has not been carried out.

We run that ablation. Our five-condition design isolates three degrees of freedom: sensor type (blind / uniform / foveated), information volume (uniform vs.\ matched-compute), and gaze policy (fixed vs.\ learned). Every condition uses the same DD-PPO learner, the same Gibson-0$+$MP3D training pool, the same LSTM backbone, and the same non-visual sensor stack; the only variables are the visual transform and the gaze policy. This lets us attribute representational and behavioural differences cleanly to input structure rather than to confounded design choices. Two accounts make divergent predictions for what a foveated agent's memory should do: a \emph{passive buffer} stores what it currently perceives, so decodable content should fall when current input is degraded; a \emph{compensatory} memory invests in internal state when current input is unreliable, so decodable content should persist over longer horizons than for an agent whose current input is always reliable.
