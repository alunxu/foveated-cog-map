...
Foveated-learned does not appear in Table~\ref{tab:path_history} because the lag-$k$ probe requires $k+1$ consecutive within-episode steps per sample, which the 4-step truncation used for cross-condition probe comparisons (§\ref{sec:methods-probing}) would eliminate entirely. On its full-episode data (171-step mean) it is flat at $R^2\approx-2.5$ across all lags, which is a different signal from the truncated numbers reported elsewhere: the flat $-2.5$ is the full-episode \emph{world-frame GPS} probe failing to generalise because foveated-learned's episodes cover a spatial range $\sim\!10^5{\times}$ larger than any other condition's probe-training-set coverage (cf.\ §\ref{sec:h2} caveat), not a claim that the hidden state contains no retrievable spatial information. On the same first-4-steps truncation used in Table~\ref{tab:probe_baseline}, foveated-learned gives compass $R^2=0.941$ and GPS $R^2=0.031$ --- the heading-dominant content pattern we return to in §\ref{sec:h3}. Because foveated-learned cannot be included in the lag-$k$ comparison at matched methodology, we do not claim either retention or compensation for foveated-learned in H1. \emph{Boundary:} our lag horizon is capped at $k=5$ by a minimum-sample threshold on the four included conditions; longer-episode runs would likely show sharper separation.
