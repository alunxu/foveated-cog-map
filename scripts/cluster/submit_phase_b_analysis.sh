...
# legacy alias needed).
SYMLINK_CMDS="cd ${NPZ_DIR} && ln -sf coarse_det.npz matched_gibson_det.npz && ln -sf blind_det.npz blind_gibson_det.npz && ln -sf uniform_det.npz uniform_gibson_det.npz && ln -sf foveated_det.npz foveated_gibson_det.npz && ln -sf coarse_det_scenes.txt matched_gibson_det_scenes.txt && ln -sf blind_det_scenes.txt blind_gibson_det_scenes.txt && ln -sf uniform_det_scenes.txt uniform_gibson_det_scenes.txt && ln -sf foveated_det_scenes.txt foveated_gibson_det_scenes.txt"

INNER_CMD="set -e; source /opt/miniconda3/etc/profile.d/conda.sh; conda activate habitat; cd /scratch/wxu/dh-spatial; export PYTHONPATH=/scratch/wxu/dh-spatial:\$PYTHONPATH; export USER=wxu; export HABITAT_DATA_DIR=/scratch/izar/wxu/habitat_data; mkdir -p ${RESULTS_DIR}; ${SYMLINK_CMDS}; cd /scratch/wxu/dh-spatial; echo STEP1_LINEAR_PROBE; for cond in coarse foveated uniform foveated_logpolar blind; do npz=${NPZ_DIR}/\${cond}_det.npz; [ -f \$npz ] || { echo SKIP \$cond no npz; continue; }; echo running analyze.py on \$cond; python -u scripts/probing/analyze.py --data \$npz --out ${RESULTS_DIR}/\${cond}_det_analysis.json --pca-dim 0 --min-steps-scene 15 2>&1 | tail -10; done; echo STEP2_MLP_PROBE; python -u scripts/probing/run_mlp_probe_proper.py --in-dir ${NPZ_DIR} --out ${RESULTS_DIR}/mlp_probe.json 2>&1 | tail -10; echo STEP3_CKA; python -u scripts/probing/unaligned_cka.py --in-dir ${NPZ_DIR} --out ${RESULTS_DIR}/cka.json 2>&1 | tail -10; echo STEP4_PROCRUSTES; python -u scripts/eval/procrustes_shape_analyze.py --in-dir ${NPZ_DIR} --out ${RESULTS_DIR}/procrustes.json --suffix _gibson_det 2>&1 | tail -10; echo STEP5_LAGK; for cond in coarse foveated uniform foveated_logpolar blind; do npz=${NPZ_DIR}/\${cond}_det.npz; [ -f \$npz ] || continue; python -u scripts/probing/lagk_all_targets.py --data \$npz --out ${RESULTS_DIR}/lagk_\${cond}.json 2>&1 | tail -5; done; echo PHASE_B_DONE; ls -la ${RESULTS_DIR}/"
