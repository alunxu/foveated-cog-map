...
# B2: MP3D generalisation probe (deterministic collection).
#
# Usage:
#   sbatch submit_probe_mp3d.sh <config_name> <ckpt_path> [num_episodes=300] [suffix=_det]
#
# Runs eval rollouts and probing on MP3D-val (held-out scenes from a dataset
# our agents did see during training). Produces <cond>_mp3d<suffix>.npz and
# <cond>_mp3d<suffix>_analysis.json.  Default suffix '_det' distinguishes
# from pre-fix stochastic collections; pass '' for no suffix.
...
NPZ_PATH="${PROBE_DIR}/${RUN_NAME}${SUFFIX}.npz"
JSON_PATH="${RESULTS_DIR}/${RUN_NAME}${SUFFIX}_analysis.json"
