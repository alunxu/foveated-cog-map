#!/bin/bash
# Quick sanity preview: run 4 analysis lenses on existing friend's blind_det.npz.
# Independent of probe-5-blind-izar — gives us a "what blind values should look like"
# baseline using friend's seed=2 ckpt that's already been probe-collected on RCP.
#
# Outputs go to *_friend.json so they don't overwrite the in-progress 5-cond
# results. Pure CPU. ETA ~10 min.
set -e

JOB_NAME="blind-friend-preview"
...
# Inline base64-encoded python: replicates mlp_rcp + lagk_rcp + skaggs in single
# pass on blind_det.npz only.
B64=$(cat <<'PYBODY' | base64 | tr -d '\n'
"""Sanity preview: 4 lenses on friend's blind_det.npz (seed=2).
Output: blind_friend_preview.json with linear+MLP+lagk+Skaggs blind row.
"""
...
NPZ = "/scratch/wxu/habitat_checkpoints_rcp/probing_data_rcp/blind_det.npz"
OUT = "/scratch/wxu/habitat_checkpoints_rcp/analysis_results/blind_friend_preview.json"
...
print(f"  shape: H={H.shape}, gps={gps.shape}, episodes={len(np.unique(ep))}")

out = {"source": "blind_seed_2_friend", "ckpt": "ckpt.49"}
...
print(f"\nWrote {OUT}")
print("\nExpected paper magnitudes (Wijmans-derived):")
print("  Linear GPS R² ~0.95  |  MLP gap ~0  |  lag-k stable >0.7  |  Skaggs ~1.25 bits")
PYBODY
)

INNER_CMD="set -e; source /opt/miniconda3/etc/profile.d/conda.sh; conda activate habitat; export PATH=/home/wxu/.local/bin:\$PATH; pip install --quiet --user scikit-learn 2>&1 | tail -2; mkdir -p ${RESULTS_DIR}; echo $B64 | base64 -d > /tmp/blind_preview.py; python -u /tmp/blind_preview.py 2>&1 | tee ${RESULTS_DIR}/blind_friend_preview.log; echo BLIND_PREVIEW_DONE"
...
echo "Submitted $JOB_NAME (CPU, ~10 min)"
