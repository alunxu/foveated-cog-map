#!/bin/bash
# Phase 1b v3: use PVC conda (has sklearn), still skip pip install, JSON before savefig.
set -e
cd /scratch/wxu/dh-spatial
source /scratch/wxu/miniconda3/etc/profile.d/conda.sh
conda activate habitat
echo "Python: $(which python), sklearn: $(python -c 'import sklearn; print(sklearn.__version__)' 2>&1 | head -1)"
