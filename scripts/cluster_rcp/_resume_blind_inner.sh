#!/bin/bash
# Inner script for dh-blind-resume — sourced via a single-arg `bash`
# invocation to avoid runai-cli quoting issues with the inline INNER_CMD.
set -ex
...
echo "=== resume state ==="
ls -la "$OUT_DIR/.habitat-resume-state.pth" || echo "WARNING: no resume state"
