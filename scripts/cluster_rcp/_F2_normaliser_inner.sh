#!/bin/bash
# Inner script for F2 (foveated + RunningMeanAndVar normaliser ENABLED).
# Tests whether disabling the input normaliser confounds the H1 finding for
# the foveated condition. Mirrors dh-probe-N hyperparams exactly (seed=0,
# 250M frames, num_environments=16, ppo.num_steps=256, no LR decay).
set -ex
...
OUT_DIR=/scratch/wxu/habitat_checkpoints_rcp/dh-fnorm
mkdir -p "$OUT_DIR"
...
ls -la "$HB_CONFIG/pointnav/ddppo_pointnav_foveated_normaliser_gibson.yaml"
head -3 "$HB_CONFIG/pointnav/ddppo_pointnav_foveated_normaliser_gibson.yaml"
